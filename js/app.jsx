...
(function () {
  ...
  const {
    genId, getToday, getMY, daysInMonth, errMsg, triggerHaptic, usePref, spendForBudget,
  } = NH.utils;
  ...
  function App({ user, toast }) {
    ...
    const [undo, setUndo] = useState(null); // {label, fn} — berlaku utk transaksi/catatan/tugas/hutang
    ...
    const [mascotOn, setMascotOn] = usePref(KEYS.mascotOn, true);
    ...
    // Peringatan budget dihitung sesuai periode (bulanan / mingguan)
    const warnings = useMemo(
      () => budgets
        .map((b) => ({ b, spent: spendForBudget(b, txns) }))
        .filter((r) => r.b.limit > 0 && r.spent / r.b.limit > 0.7)
        .slice(0, 3),
      [budgets, txns]
    );
    ...
    useEffect(() => {
      if (!undo) return;
      const tm = setTimeout(() => setUndo(null), 6000);
    ...
    const delNote = async (id) => {
      ...
      try {
        ...
        if (n) setUndo({
          label: 'Catatan dihapus',
          fn: async () => {
            const { error } = await sb.from('notes').insert({
              id: n.id, user_id: user.id, title: n.title, content: n.content, tags: n.tags || [], pinned: !!n.pinned,
            });
            if (error) throw error;
            setNotes((p) => [n, ...p]);
          },
        ...
        if (t0) setUndo({
          ...
          fn: async () => {
            const { error } = await sb.from('todos').insert({
              id: t0.id, user_id: user.id, title: t0.title, completed: !!t0.completed, deadline: t0.deadline, budget: t0.budget || 0,
            });
            if (error) throw error;
            setTodos((p) => [t0, ...p]);
          },
    ...
    const addDebt = async (d) => {
      ...
      try {
        ...
        toast('Hutang/piutang disimpan \u2713', 'ok');
        return true;
    ...
    const delDebt = async (id) => {
      const d0 = debts.find((x) => x.id === id);
      try {
        const { error } = await sb.from('debts').delete().eq('id', id);
        if (error) throw error;
        setDebts((p) => p.filter((x) => x.id !== id));
        if (d0) setUndo({
          label: 'Hutang/piutang dihapus',
          fn: async () => {
            const { error } = await sb.from('debts').insert({
              id: d0.id, user_id: user.id, name: d0.name, dtype: d0.dtype, amount: d0.amount, due_date: d0.dueDate, note: d0.note || '', status: d0.status,
            });
            if (error) throw error;
            setDebts((p) => [d0, ...p]);
          },
      ...
      <div style={{ minHeight: '100vh' }} onTouchStart={handleTouchStart} onTouchEnd={handleTouchEnd}>
        ...
        <div className="page anim-page" key={pageKey}>
          {tab === 'home' && (
            <NH.Home
              ...
              mascotOn={mascotOn} warnings={warnings}
            />
          )}
          {tab === 'notes' && (
            <NH.Notes
          ...
          {tab === 'settings' && (
            <NH.Settings
              ...
              mascotOn={mascotOn} setMascotOn={setMascotOn}
        ...
        {modal === 'scanReceipt' && (
          <NH.ScanModal
            ...
            learnedKw={learnedKw} setLearnedKw={setLearnedKw} addTxn={addTxn}
          />
        )}
        ...
        {undo && (
          <div className="undo-bar">
            <div className="undo-card">
              <span>🗑️ {undo.label}</span>
              <button className="bold accent" style={{ padding: '4px 10px' }}
                onClick={async () => {
                  const fn = undo.fn;
                  setUndo(null);
                  try { await fn(); toast('Dipulihkan \u2713', 'ok'); }
                  catch (e) { toast('Gagal memulihkan: ' + errMsg(e)); }
                }}>UNDO</button>
            </div>
          </div>
        )}
